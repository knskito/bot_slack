...
module.exports = (robot) ->
    ...
    robot.hear /(村上|隆|takashipom)/, (msg) ->
        arr = ["なんだおめ。突然人の名前出して知った風な口ききやがって。", "お前みたいな自意識過剰な若者の適当発言に耳を傾ける価値無し！", "当事者意識も無く、発言に責任を負う訳でもなく、外から偉そうにあれこれ語る自称批評家やキュレーター達のなんと多いことか。こういった発言からも日本アートシーンの病理病巣が垣間見えますね。", "なんでも良いけど、言いたいことがあるならまず自分から名乗るのが礼儀ってもんだろ。", "ははは。相変わらず国内では嫌われてるなぁ、わし。", "いつもコソコソとわしの悪口ご苦労さまです！"]
        cont = Math.floor(Math.random() * arr.length)
        msg.send arr[cont]
